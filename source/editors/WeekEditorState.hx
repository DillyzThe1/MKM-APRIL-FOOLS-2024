	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			...
			else if (sender == songsInputText)
			{
				...
				while (splitText.length < weekFile.songs.length)
				{
					weekFile.songs.pop();
				}
				...
				for (i in 0...splitText.length)
				{
					...
					{ // Add new song
						weekFile.songs.push([splitText[i], 'dad', [146, 113, 253]]);
					}
					else
					{ // Edit song
						weekFile.songs[i][0] = splitText[i];
						weekFile.songs[i][4] = '${splitText[i].toLowerCase().replace(' ', '-')}-start';
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function addFreeplayUI()
	{
		...
		hideSongFromStoryMode = new FlxUICheckBox(10, hideFreeplayCheckbox.y + 30, null, null, "Hide from Story Mode?", 100);
		hideSongFromStoryMode.checked = weekFile.songs[curSelected].length >= 4 ? weekFile.songs[curSelected][3] : false;
		hideSongFromStoryMode.callback = function()
		{
			weekFile.songs[curSelected][3] = hideSongFromStoryMode.checked;
		};

		unlockerKey = new FlxUIInputText(10, hideSongFromStoryMode.y + 48, 200,
