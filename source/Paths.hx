...
class Paths
{
	...
	{
		return getPath('data/${checkStr(key)}.json', TEXT, library);
	}
	...
	{
		return checkStr(path.toLowerCase().replace(' ', '-'));
	}
	...
	{
		return modFolders('data/' + checkStr(key) + '.json');
	}
