...
class FunkinLua
{
	...
	public function new(script:String)
	{
		...
		Lua_helper.add_callback(lua, "shaderByString", function(bruhhhhh:String)
		{
			if (bruhhhhh == '' || bruhhhhh == null)
				return;
			// https://github.com/HaxeFlixel/flixel-demos/blob/dev/Effects/Filters/source/PlayState.hx
			/*PlayState.instance.camGame.setFilters([
					// new openfl.filters.BlurFilter(),
					new openfl.filters.ShaderFilter(new shaders.Grain()),
					new openfl.filters.ShaderFilter(new shaders.Hq2x()),
					new openfl.filters.ShaderFilter(new shaders.Scanline()),
					new openfl.filters.ShaderFilter(new shaders.Tiltshift())
				]); */

			PlayState.instance.camGameFilters.push(PlayState.instance.filterMap.get(bruhhhhh).filter);
		});

		///////////////////////// SSHHHAHAAAADEERER PASTEEEEEEE
		...
		Lua_helper.add_callback(lua, "callMethod", function(methodd:String, arguments:Array<String>)
		{
			callMethod(methodd, arguments);
		});
		Lua_helper.add_callback(lua, "callObjectMethod", function(object:String, methodd:String, arguments:Array<String>)
		{
			callObjectMethod(object, methodd, arguments);
		});
		Lua_helper.add_callback(lua, "callMethodFromClass", function(clazz:String, methodd:String, arguments:Array<String>)
		{
			callMethodFromClass(clazz, methodd, arguments);
		});
		Lua_helper.add_callback(lua, "callObjectMethodFromClass", function(clazz:String, object:String, methodd:String, arguments:Array<String>)
		{
			callObjectMethodFromClass(clazz, object, methodd, arguments);
		});
	...
	// find the object, find the method, and execute with the arguments
	function callObjectMethod(object:String, method:String, arguments:Array<String>)
	{
		var a:Array<String> = object.split('.');
		var b:Dynamic = null;
		if (PlayState.instance.modchartSprites.exists(a[0]))
			b = PlayState.instance.modchartSprites.get(a[0]);
		else if (PlayState.instance.modchartTexts.exists(a[0]))
			b = PlayState.instance.modchartTexts.get(a[0]);
		else
			b = Reflect.getProperty(getInstance(), a[0]);

		if (a.length > 1)
			for (i in 1...a.length - 1)
				b = Reflect.getProperty(b, a[i]);

		return Reflect.callMethod(b, Reflect.field(b, method), arguments);
	}

	function callMethodFromClass(clazz:String, method:String, arguments:Array<String>)
	{
		return Reflect.callMethod(Type.resolveClass(clazz), Reflect.field(Type.resolveClass(clazz), method), arguments);
	}

	function callObjectMethodFromClass(clazz:String, object:String, method:String, arguments:Array<String>)
	{
		var a:Array<String> = object.split('.');
		var b:Dynamic = Reflect.getProperty(Type.resolveClass(clazz), a[0]);
	...
	function callMethod(method:String, arguments:Array<String>)
	{
		return Reflect.callMethod(getInstance(), Reflect.field(getInstance(), method), arguments); // Reflect.setProperty(getInstance(), variable, value);
	}
...
class ModchartSprite extends FlxSprite
{
	...
	{
		super.update(e);
		if (this.animation.curAnim != null && this.animation.curAnim.name != null)
			this.animName = this.animation.curAnim.name;
		if (this.animation.curAnim != null)
			this.animFinished = this.animation.curAnim.finished;
	}
}
